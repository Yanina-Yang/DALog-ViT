...
class PostGeluAdaptiveQuantLinear(AsymmetricallyBatchingQuantLinear):
    """
    Adaptive quantization linear layer for GELU activation
    Integrates DALog logarithmic quantization advantages to implement hybrid quantization strategy
    Inherits from AsymmetricallyBatchingQuantLinear to obtain complete asymmetric quantization functionality
    """
    def __init__(self, 
                 in_features: int,
                 out_features: int,
                 ...
                 n_V = 1, 
                 token_channel_wise=False,
                 use_gelu_adaptive=True,  # Whether to enable GELU adaptive quantization
                 importance_threshold=0.2,
                 enable_preprocessing=True):  # New: Whether to enable activation preprocessing
        super().__init__(in_features, out_features, bias=bias, mode=mode, w_bit=w_bit, a_bit=a_bit,
                         metric=metric, calib_batch_size=calib_batch_size, search_round=search_round, 
                         eq_n=eq_n, n_V=n_V, token_channel_wise=token_channel_wise)
        
        self.use_gelu_adaptive = use_gelu_adaptive
        
        if use_gelu_adaptive:
            # Replace the original asymmetric quantizer with GELU adaptive quantizer, pass configuration parameters
            del self.a_quantizer
            self.a_quantizer = HAQSQuantizer(
                n_bits=a_bit, 
                symmetric=False, 
                channel_wise=False,
                importance_threshold=importance_threshold,  # Pass importance threshold
                enable_preprocessing=enable_preprocessing  # Pass preprocessing switch
            )

    
    def quant_input(self, x):
        """Rewrite activation quantization method to support GELU adaptive quantization"""
        if self.use_gelu_adaptive:
            return self.a_quantizer(x)
        else:
            return super().quant_input(x)
    

    def hyperparameter_searching(self):
        if not self.use_gelu_adaptive:
            super().hyperparameter_searching()
            return

        # --- Customized search process for GELU adaptive quantization ---

        # 1. Initialize calibration parameters
        self._initialize_calib_parameters()

        # 2. Explicitly initialize HAQSQuantizer
        # This is the core of solving incompatibility issues: execute correct initialization at the right time
        if not self.a_quantizer.inited:
            sample_x = self.raw_input[:min(32, self.raw_input.shape[0])].cuda()
            self.a_quantizer.init_from(sample_x)
            self.a_quantizer.inited = True

        # 3. Only perform hyperparameter search for weight
        # Activation quantizer is self-initializing and does not need search
        weight_scale_candidates, weight_zero_point_candidates = self.calculate_percentile_weight_candidates()
        
        # Use parent class method to search weight parameters, this is compatible
        self._search_best_w_scale_self(weight_scale_candidates, weight_zero_point_candidates)
