...
def wrap_modules_in_net(model, cfg, reparam=False):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.Linear):
            ...
            # Modified: Check if GELU adaptive quantization is enabled and determine if this is a post-GELU layer
            use_gelu_adaptive = getattr(cfg, 'use_gelu_adaptive', False)
            # More precise post-GELU layer detection logic to avoid false positives
            is_after_gelu = False
            if use_gelu_adaptive:
                if 'mlp.fc2' in name:
                    is_after_gelu = True
            
            if use_gelu_adaptive and is_after_gelu:
                # Use GELU adaptive quantization linear layer - pass configuration parameters
                new_module = PostGeluAdaptiveQuantLinear(
                    in_features=linear_kwargs['in_features'],
                    out_features=linear_kwargs['out_features'],
                    bias=linear_kwargs['bias'],
                    mode=linear_kwargs['mode'],
                    w_bit=linear_kwargs['w_bit'],
                    a_bit=linear_kwargs['a_bit'],
                    metric=linear_kwargs['metric'],
                    calib_batch_size=linear_kwargs['calib_batch_size'],
                    search_round=linear_kwargs['search_round'],
                    eq_n=linear_kwargs['eq_n'],
                    n_V=linear_kwargs['n_V'],
                    token_channel_wise=linear_kwargs['token_channel_wise'],
                    use_gelu_adaptive=True,
                    importance_threshold=getattr(cfg, 'importance_threshold', 0.5),  # Pass importance threshold from config file
                    enable_preprocessing=getattr(cfg, 'enable_preprocessing', True)  # Pass preprocessing switch from config file
                )
            elif cur_a_bit == cfg.w_bit and reparam and ('qkv' in name or 'reduction' in name or 'fc1' in name):
                idxx = father_name.rfind('.')
                idxx = 0 if idxx == -1 else idxx
                grandfather_name = father_name[:idxx]
                grandfather_module = None
                if grandfather_name in module_dict:
                    grandfather_module = module_dict[grandfather_name]
                elif grandfather_name == '':
                    grandfather_module = model
                
                new_module = AsymmetricallyChannelWiseBatchingQuantLinear(
                    **linear_kwargs, 
                )
                if 'qkv' in name and grandfather_module is not None and hasattr(grandfather_module, 'norm1'):
                    new_module.prev_layer = grandfather_module.norm1
                if 'fc1' in name and grandfather_module is not None and hasattr(grandfather_module, 'norm2'):
                    new_module.prev_layer = grandfather_module.norm2
                if 'reduction' in name and hasattr(father_module, 'norm'):
...
def wrap_reparamed_modules_in_net(model):
    module_dict = {}
    for name, module in tqdm(model.named_modules(), desc="Reparameterizing"):
        # PostGeluAdaptiveQuantLinear uses special quantizers and should not be reparameterized by this function, skip directly
